...
mod test {
    ...
    fn state_adapter_maps() {
        let serialized_root = [
            16, 0, 0, 0, 0, 0, 10, 0, 18, 0, 8, 0, 7, 0, 12, 0, 10, 0, 0, 0, 0, 0, 0, 0, 32, 0, 0,
            0, 12, 0, 0, 0, 0, 0, 6, 0, 8, 0, 4, 0, 6, 0, 0, 0, 4, 0, 0, 0, 252, 255, 255, 255, 4,
            0, 4, 0, 4, 0, 0, 0,
        ];
        // let my_string = Root::default().to_string();  // `parse()` works with `&str` and `String`!
        // let my_int = my_string.parse::<i32>().unwrap();
        // println!("{:?}",FlatBufferBuilder(serialized_root.to_vec()))
        // assert_eq!(map_state(&Root::default()), serialized_root.to_string())
    }
    //
    // #[test]
    // fn apply_click_action_on_default_state() {
    //     let root = RootReducer::reduce(&Root::default(), &click_action(String::from("num")));
    //     let serialized_root = [
