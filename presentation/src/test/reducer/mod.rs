...
mod test {
    ...
    //     let root = RootState::default();
    //     assert!(matches!(root.page, MainPageTypeState {ref page} if page.text.count == String::from("")) );
    //     let new_root = RootReducer::reduce(&root, &click_action());
    //     assert!(matches!(new_root.page, MainPageTypeState {page} if page.text.count == String::from(""))   );
    // }

    fn click_action() -> RootAction {
        RootAction {
            user: UserAction::None,
            page: MainPageTypeAction { page: ButtonClicked{ name: "".to_string() },
        }
    }
}
}
